...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Convert activity to可以发送给Zep的文本描述
        
        Uses natural languagedescription format，让Zep能够从中提取实体和关系
...
class ZepGraphMemoryUpdater:
    ...
    监控模拟的actions日志文件，Real-time update new agent activities to Zep graph中。
    Grouped by platform，每累积BATCH_SIZE条活动后批量发送到Zep。
    ...
    # 批量发送大小（items per platform before sending）
    BATCH_SIZE = 5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Grouped by platform的活动缓冲区（每个平台各自累积到BATCH_SIZE后批量发送）
...
class ZepGraphMemoryManager:
    """
    管理多个模拟的ZepGraph memory updater
    ...
    def create_updater(cls, simulation_id: str, graph_id: str) -> ZepGraphMemoryUpdater:
        ...
        with cls._lock:
            ...
            logger.info(f"创建Graph memory updater: simulation_id={simulation_id}, graph_id={graph_id}")
            return updater
    ...
    def stop_updater(cls, simulation_id: str):
        ...
        with cls._lock:
            if simulation_id in cls._updaters:
                ...
                logger.info(f"已停止Graph memory updater: simulation_id={simulation_id}")
    ...
    def stop_all(cls):
        ...
        with cls._lock:
            if cls._updaters:
                ...
                cls._updaters.clear()
            logger.info("已停止所有Graph memory updater")
