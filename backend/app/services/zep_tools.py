"""
Zep检索工具服务
Wraps图谱搜索、节点读取、边查询等工具，for Report Agent use

Core检索工具（optimized）：
1. InsightForge（深度洞察检索）- Most powerful hybrid检索，Auto-generates sub-questions for multi-dimensional检索
2. PanoramaSearch（广度搜索）- Get full picture，including expired内容
3. QuickSearch（Simple搜索）- Quick检索
"""
...
class PanoramaResult:
    """
    广度搜索结果 (Panorama)
    包含所有相关信息，including expired内容
    """
    query: str
    
    # 全部节点
    all_nodes: List[NodeInfo] = field(default_factory=list)
    # 全部边（including expired的）
...
class ZepToolsService:
    ...
    2. panorama_search - 广度搜索（Get full picture，including expired内容）
    3. quick_search - Simple搜索（Quick检索）
    4. interview_agents - 深度采访（Interview simulated Agents，获取多视角观点）
    ...
    def search_graph(
        ...
        如果Zep Cloud的search API不可用，则降级为Local keyword匹配。
    ...
    def _local_search(
        ...
        """
        Local keyword匹配搜索（作为Zep Search API的降级方案）
    ...
    def insight_forge(
        ...
        Most powerful hybrid检索函数，自动分解问题并多维度检索：
        1. 使用LLM将问题分解为多个子问题
        2. 对每个子问题进行语义搜索
        3. 提取相关实体并获取其详细信息
        4. 追踪关系链
...
返回JSON格式的子问题列表。"""
    ...
    def panorama_search(
        ...
        Get full picture视图，包括所有相关内容和历史/过期信息：
        1. 获取所有相关节点
        2. 获取所有边（包括已过期/失效的）
        3. 分类整理当前有效和历史信息
    ...
    def quick_search(
        ...
        Returns:
            SearchResult: 搜索结果
        """
        logger.info(f"QuickSearch Simple搜索: {query[:50]}...")
        ...
        try:
            ...
            for agent_idx in selected_indices:
                interviews_request.append({
                    "agent_id": agent_idx,
                    "prompt": optimized_prompt  # 使用optimized的prompt
