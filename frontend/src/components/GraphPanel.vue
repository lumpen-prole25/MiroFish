  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" title="{{ $t('graph.tooltipRefresh') }}">
          <span class="icon-refresh" :class="{ 'spinning': loading }">↻</span>
          <span class="btn-text">Refresh</span>
        </button>
        <button class="tool-btn" @click="$emit('toggle-maximize')" title="{{ $t('graph.tooltipMaximize') }}">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? '{{ $t('graph.buildingHintSimulating') }}' : '{{ $t('graph.buildingHintUpdating') }}' }}
        </div>
